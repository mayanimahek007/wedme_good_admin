...
const Food = () => {
  ...
  const getPhotographerData = () => {
    axios.get('https://wedmegood-backend.onrender.com/api/v1/food/get')
      .then(response => {
  ...
  const addPhoto = async () => {
    ...
    formData.append("foodName", photographerName);
    formData.append("foodPriceInfo", photographerPriceInfo);
    formData.append("foodPrice", photographerPrice);
    formData.append("foodNumber", photographerNumber);
    formData.append("foodDetails", photographerDetails);
    formData.append("foodEmail", photographerEmail);
    formData.append("foodDate", photographerDate);

    photographerMedia.forEach((image, index) => {
      formData.append(`foodMedia`, image); // Multiple files support
    });

    try {
      setLoading(true); // ✅ Loading state ON
      const res = await axios.post("https://wedmegood-backend.onrender.com/api/v1/food/create", formData, {
  ...
  const handleEdit = (venue) => {
    ...
    setPhotographerName(venue.foodName);
    setPhotographerPriceInfo(venue.foodPriceInfo);
    setPhotographerPrice(venue.foodPrice);
    setPhotographerNumber(venue.foodNumber);
    setPhotographerDetails(venue.foodDetails);
    setPhotographerEmail(venue.foodEmail);
    setPhotographerDate(venue.foodDate);
    setPhotographerMedia(Array.isArray(venue.foodMedia) ? venue.foodMedia : [venue.foodMedia]);
  ...
  const deletePhotographer = async (id) => {
    ...
    try {
      setLoading(true);
      const res = await axios.delete(`https://wedmegood-backend.onrender.com/api/v1/food/delete/${id}`);
  ...
  const updateVenueData = async (id) => {
    ...
    photographerMedia.forEach((image) => {
      if (typeof image !== 'string') {  // Only append if it's a new file
        formData.append(`foodMedia`, image);
      }
    });
    try {
      setLoading(true);
      const res = await axios.put(`https://wedmegood-backend.onrender.com/api/v1/food/update/${id}`, formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });
              ...
              <div class="mb-npx">
                <div class="row align-items-center">
                  <div class="col-sm-6 col-12 mb-4 mb-sm-0">
                    <h1 class="h2 mb-0 ls-tight">Food</h1>
                  </div>
