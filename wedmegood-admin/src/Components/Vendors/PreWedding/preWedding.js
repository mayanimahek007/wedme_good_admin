...
const PreWedding = () => {
  ...
  const getPhotographerData = () => {
    axios.get('https://wedmegood-backend.onrender.com/api/v1/prewedding/get')
      .then(response => {
  ...
  const addPhoto = async () => {
    ...
    try {
      ...
      const res = await axios.post("https://wedmegood-backend.onrender.com/api/v1/prewedding/create", formData, {
  ...
  const deletePhotographer = async (id) => {
    ...
    try {
      setLoading(true);
      const res = await axios.delete(`https://wedmegood-backend.onrender.com/api/v1/prewedding/delete/${id}`);

      if (res.status === 200) {
        getPhotographerData(); // Refresh list after deletion
                        toast.success('Prewedding Deleted Successfully!');
  ...
  const updateVenueData = async (id) => {
    ...
    try {
      setLoading(true);
      const res = await axios.put(`https://wedmegood-backend.onrender.com/api/v1/prewedding/update/${id}`, formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });

      if (res.status === 200) {
        getPhotographerData();
                        toast.success('Prewedding Updated Successfully!');
