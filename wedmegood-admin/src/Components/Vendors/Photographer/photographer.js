...
const Photographer = () => {
  ...
  const addPhoto = async () => {
    ...
    try {
      ...
      const res = await axios.post("https://wedmegood-backend.onrender.com/api/v1/photographer/create", formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });

      // Check if the response status is successful (e.g., 200 or 201)
      if (res.status === 201) {
                toast.success('Photographer Create Successfully!');
  ...
  const deletePhotographer = async (id) => {
    ...
    try {
      setLoading(true);
      const res = await axios.delete(`https://wedmegood-backend.onrender.com/api/v1/photographer/delete/${id}`);
  ...
  const updateVenueData = async (id) => {
    ...
    try {
      setLoading(true);
      const res = await axios.put(`https://wedmegood-backend.onrender.com/api/v1/photographer/update/${id}`, formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });
