...
const PlanningDecor = () => {
  ...
  const getPhotographerData = () => {
    axios.get('https://wedmegood-backend.onrender.com/api/v1/planningDecor/get')
      .then(response => {
  ...
  const addPhoto = async () => {
    ...
    formData.append("planningName", photographerName);
    formData.append("planningPriceInfo", photographerPriceInfo);
    formData.append("planningPrice", photographerPrice);
    formData.append("planningNumber", photographerNumber);
    formData.append("planningDetails", photographerDetails);
    formData.append("planningEmail", photographerEmail);
    formData.append("planningDate", photographerDate);

    photographerMedia.forEach((image, index) => {
      formData.append(`planningMedia`, image); // Multiple files support
    });

    try {
      setLoading(true); // ✅ Loading state ON
      const res = await axios.post("https://wedmegood-backend.onrender.com/api/v1/planningDecor/create", formData, {
  ...
  const handleEdit = (venue) => {
    ...
    setPhotographerName(venue.planningName);
    setPhotographerPriceInfo(venue.planningPriceInfo);
    setPhotographerPrice(venue.planningPrice);
    setPhotographerNumber(venue.planningNumber);
    setPhotographerDetails(venue.planningDetails);
    setPhotographerEmail(venue.planningEmail);
    setPhotographerDate(venue.planningDate);
    setPhotographerMedia(Array.isArray(venue.planningMedia) ? venue.planningMedia : [venue.planningMedia]);
  ...
  const deletePhotographer = async (id) => {
    ...
    try {
      setLoading(true);
      const res = await axios.delete(`https://wedmegood-backend.onrender.com/api/v1/planningDecor/delete/${id}`);
  ...
  const updateVenueData = async (id) => {
    ...
    photographerMedia.forEach((image) => {
      if (typeof image !== 'string') {  // Only append if it's a new file
        formData.append(`planningMedia`, image);
      }
    });
    try {
      setLoading(true);
      const res = await axios.put(`https://wedmegood-backend.onrender.com/api/v1/planningDecor/update/${id}`, formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });
              ...
              <div class="mb-npx">
                <div class="row align-items-center">
                  <div class="col-sm-6 col-12 mb-4 mb-sm-0">
                    <h1 class="h2 mb-0 ls-tight">Planning & Decor</h1>
                  </div>
            ...
            <div class="container-fluid">
              ...
              <div class="card shadow border-0 mb-7">
                <div class="card-header">
                  <h5 class="mb-0">Planning & Decor</h5>
                </div>
                    ...
                    <tbody>
                      {photographers.length > 0 ? (
                        photographers.map((photographers, index) => (
                          <tr key={index}>
                            <td>{photographers.planningName}</td>
                            <td>{new Date(photographers.planningDate).toLocaleDateString()}</td>
                            <td>{photographers.planningEmail}</td>
                            <td>{photographers.planningNumber}</td>
                            <td>${photographers.planningPrice}</td>
                            <td>{photographers.planningPriceInfo}</td>
                            <td>{photographers.planningDetails}</td>
                            {/* <td className="text-truncate" style={{ maxWidth: "150px", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{photographer.planningMedia}</td> */}
                            <td style={{ minWidth: '195px' }}>
                              <div className="d-flex align-items-center">
                                {Array.isArray(photographers.planningMedia) ? (
                                  photographers.planningMedia.slice(0, 3).map((img, idx) => (
                                    <img
                                      key={idx}
                                  ...
                                  <img
                                    src={typeof photographers.planningMedia === 'string' ? photographers.planningMedia : URL.createObjectURL(photographers.planningMedia)}
                                    alt="Wedding"
                                    className="avatar avatar-md rounded-circle border"
                                    style={{ width: "40px", height: "40px", objectFit: "cover", border: "2px solid #fff" }}
                                  />
                                )}
                                {Array.isArray(photographers.planningMedia) && photographers.planningMedia.length > 3 && (
                                  <span
                                  ...
                                  >
                                    +{photographers.planningMedia.length - 3}
                                  </span>
                                )}
                              </div>
                            </td>
        ...
        <div className="modal fade show d-block" tabIndex="-1" style={{ background: 'rgba(0,0,0,0.5)' }}>
          <div className="modal-dialog">
            <div className="modal-content">
              ...
              <h4 className="modal-title">{serviceId ? "Update Planning & Decor" : "Create Planning & Decor"}</h4>
